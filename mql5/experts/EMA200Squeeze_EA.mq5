//+------------------------------------------------------------------+
//|                                        EMA200Squeeze_EA.mq5      |
//|                                         AlgoStrategies           |
//|  200 EMA Squeeze Strategy with 3 Partial TPs, Trailing SL,       |
//|  and on-chart P&L Dashboard                                       |
//+------------------------------------------------------------------+
#property copyright "AlgoStrategies"
#property link      ""
#property version   "3.00"
#property description "200 EMA Squeeze: reverse on exit, ADX filter, ST trail remaining qty, 3 partial TPs, trailing SL, P&L dashboard"
...
input long             InpMagic         = 200200;                     // Magic Number
input string           InpComment       = "EMA200Squeeze";            // Order Comment
...
{
   ObjectsDeleteAll(0, DASH_PREFIX);  // Removes EMA segments, dashboard, arrows, TP lines, TSL
   RemoveTPLines();
   RemoveSignalArrows();
   ChartRedraw(0);
}
...
{
   //--- Only ONE TP can fire per bar (sequential booking, not simultaneous)
...
//+------------------------------------------------------------------+
//| Process Trailing Stop Loss (update SL on position + detect hits) |
//+------------------------------------------------------------------+
   ...
   if(g_TradeState == 1) // Long
   {
      ...
      if(highPrice >= triggerPrice)
      {
         ...
         if(!g_TSLActive || newStop > g_TrailStop)
         {
            g_TrailStop = newStop;
            g_TSLActive = true;
            //--- Modify the actual SL on the broker position
            ModifyPositionSL(newStop);
            Print("TSL updated LONG: newSL=", newStop, " (trail offset ", InpTSLOffsetPct, "%)");
         }
      }
      //--- Check if SL is hit
      if(g_TSLActive && lowPrice <= g_TrailStop)
      {
         ...
         Print("TSL HIT for LONG at ", g_TrailStop);
      }
   }
   else if(g_TradeState == -1) // Short
   {
      ...
      if(lowPrice <= triggerPrice)
      {
         ...
         if(!g_TSLActive || newStop < g_TrailStop)
         {
            ...
            //--- Modify the actual SL on the broker position
            ModifyPositionSL(newStop);
            Print("TSL updated SHORT: newSL=", newStop, " (trail offset ", InpTSLOffsetPct, "%)");
         }
      }
      //--- Check if SL is hit
      if(g_TSLActive && highPrice >= g_TrailStop)
      {
         ...
         Print("TSL HIT for SHORT at ", g_TrailStop);
      }
   }
}
...
//+------------------------------------------------------------------+
//| Modify position SL (for trailing stop)                           |
//+------------------------------------------------------------------+
void ModifyPositionSL(double newSL)
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      //--- Only modify if new SL is better (tighter)
      if(g_TradeState == 1 && newSL > curSL)  // Long: SL should go UP
      {
         if(g_Trade.PositionModify(ticket, newSL, 0))
         {
            g_StopLoss = newSL;
            Print("Modified LONG SL: ", curSL, " → ", newSL);
         }
         else
            Print("ModifyPositionSL FAILED for long: retcode=", g_Trade.ResultRetcode());
         return;
      }
      else if(g_TradeState == -1 && newSL < curSL)  // Short: SL should go DOWN
      {
         if(g_Trade.PositionModify(ticket, newSL, 0))
         {
            g_StopLoss = newSL;
            Print("Modified SHORT SL: ", curSL, " → ", newSL);
         }
         else
            Print("ModifyPositionSL FAILED for short: retcode=", g_Trade.ResultRetcode());
         return;
      }
   }
}
...
void CreateDashboard()
{
   ...
   CreateLabel(DASH_PREFIX + "TITLE", x + cellW / 2, y, "200 EMA Squeeze", 10, clrDodgerBlue);

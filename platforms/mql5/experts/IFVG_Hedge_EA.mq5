...
enum HedgeState {                                                  // Hedge state enum
   Hedge_None,                                                     // No positions
   Hedge_Full,                                                     // Both Buy + Sell open
   Hedge_BuyOnly,                                                  // Only Buy running
   Hedge_SellOnly                                                  // Only Sell running
};
...
input int    maxTradesPerFVG    = 2;                               // Maximum trades per FVG (if LimitedTrades)
...
//+------------------------------------------------------------------+
//| Hedge tracking variables                                         |
//+------------------------------------------------------------------+
HedgeState currentHedgeState = Hedge_None;                         //--- Current hedge state
ulong      buyTicket  = 0;                                        //--- Buy leg ticket
ulong      sellTicket = 0;                                        //--- Sell leg ticket
...
double CalcLot() {
   ...
   double slMoney = (sl_pts * _Point / tickSize) * tickValue;     //--- SL cost per 1 lot
...
bool IsPositionOpen(ulong ticket) {
   if (ticket == 0) return false;                                  //--- No ticket
   for (int i = PositionsTotal() - 1; i >= 0; i--) {              //--- Iterate positions
      if (PositionGetTicket(i) == ticket) {                        //--- Check ticket
...
bool ClosePositionByTicket(ulong ticket) {
   if (ticket == 0) return false;                                  //--- No ticket
   if (!IsPositionOpen(ticket)) return false;                      //--- Not open
   bool result = obj_Trade.PositionClose(ticket);                  //--- Close
   if (prt) {
      if (result)
         Print("Hedge: Closed position ticket=", ticket);          //--- Log success
      else
         Print("Hedge: Failed to close ticket=", ticket, " error=", GetLastError()); //--- Log fail
   }
   return result;                                                  //--- Return result
}

//+------------------------------------------------------------------+
//| Open full hedge (Buy + Sell) — no SL/TP on hedge legs            |
//+------------------------------------------------------------------+
bool OpenFullHedge() {
   double lot = CalcLot();                                         //--- Calc lot
   ...
   //--- Open Buy leg (no SL/TP — held until signal)
   bool buyOk = obj_Trade.Buy(lot, _Symbol, Ask, 0, 0, "Hedge Buy"); //--- Open buy
   if (buyOk) {
      buyTicket = obj_Trade.ResultOrder();                         //--- Store ticket
      if (prt) Print("Hedge: Opened Buy leg ticket=", buyTicket, " lot=", lot, " at ", Ask); //--- Log
   } else {
      if (prt) Print("Hedge: Failed to open Buy leg, error=", GetLastError()); //--- Log fail
      return false;                                                //--- Return fail
   }

   //--- Open Sell leg (no SL/TP — held until signal)
   bool sellOk = obj_Trade.Sell(lot, _Symbol, Bid, 0, 0, "Hedge Sell"); //--- Open sell
   if (sellOk) {
      sellTicket = obj_Trade.ResultOrder();                        //--- Store ticket
      if (prt) Print("Hedge: Opened Sell leg ticket=", sellTicket, " lot=", lot, " at ", Bid); //--- Log
   } else {
      if (prt) Print("Hedge: Failed to open Sell leg, error=", GetLastError()); //--- Log fail
      ClosePositionByTicket(buyTicket);                            //--- Rollback buy
      buyTicket = 0;                                               //--- Reset
      return false;                                                //--- Return fail
   }

   currentHedgeState = Hedge_Full;                                 //--- Set state
   if (prt) Print("Hedge: Full hedge opened. Buy=", buyTicket, " Sell=", sellTicket); //--- Log
   return true;                                                    //--- Return success
}

//+------------------------------------------------------------------+
//| Update hedge state by checking live positions                    |
//+------------------------------------------------------------------+
void UpdateHedgeState() {
   bool buyOpen  = IsPositionOpen(buyTicket);                      //--- Check buy
   bool sellOpen = IsPositionOpen(sellTicket);                     //--- Check sell
   HedgeState prevState = currentHedgeState;                       //--- Save prev state

   if (buyOpen && sellOpen)
      currentHedgeState = Hedge_Full;                              //--- Both open
   else if (buyOpen && !sellOpen) {
      currentHedgeState = Hedge_BuyOnly;                           //--- Buy only
      sellTicket = 0;                                              //--- Clear sell
   } else if (!buyOpen && sellOpen) {
      currentHedgeState = Hedge_SellOnly;                          //--- Sell only
      buyTicket = 0;                                               //--- Clear buy
   } else {
      currentHedgeState = Hedge_None;                              //--- None
      buyTicket = 0;                                               //--- Clear both
      sellTicket = 0;
   }

   //--- Remaining leg hit TP/SL → re-hedge immediately
   if (currentHedgeState == Hedge_None &&
       (prevState == Hedge_BuyOnly || prevState == Hedge_SellOnly)) {
      if (prt) Print("Hedge: Remaining leg closed (TP/SL hit). Re-hedging..."); //--- Log
      OpenFullHedge();                                             //--- Re-hedge
   }

   //--- Both legs closed (e.g. manual close) → re-hedge
   if (currentHedgeState == Hedge_None && prevState == Hedge_Full) {
      if (prt) Print("Hedge: Both legs closed. Re-hedging...");    //--- Log
      OpenFullHedge();                                             //--- Re-hedge
   }
}

//+------------------------------------------------------------------+
//| Close losing leg and apply SL/TP to winning leg                  |
//| SL/TP use current market price (signal time) to match original   |
//+------------------------------------------------------------------+
void CloseLosingLeg(bool isBuySignal, double fvgLow, double fvgHigh) {
   if (currentHedgeState != Hedge_Full) return;                    //--- Only from full hedge

   double Ask = NormalizeDouble(SymbolInfoDouble(_Symbol, SYMBOL_ASK), _Digits); //--- Current ask
   double Bid = NormalizeDouble(SymbolInfoDouble(_Symbol, SYMBOL_BID), _Digits); //--- Current bid

   if (isBuySignal) {
      //--- Buy signal → close Sell (loser), keep Buy (winner)
      if (prt) Print("Hedge: BUY signal — closing Sell leg ticket=", sellTicket); //--- Log
      ClosePositionByTicket(sellTicket);                           //--- Close sell
      sellTicket = 0;                                              //--- Clear
      currentHedgeState = Hedge_BuyOnly;                           //--- Update state

      //--- SL/TP from current market price, identical to original EA
      if (IsPositionOpen(buyTicket)) {
         double newSL = NormalizeDouble(fvgLow - sl_pts * _Point, _Digits); //--- SL same as original
         double newTP = NormalizeDouble(Ask + tp_pts * _Point, _Digits);    //--- TP same as original
         obj_Trade.PositionModify(buyTicket, newSL, newTP);        //--- Modify
         if (prt) Print("Hedge: Buy leg SL=", newSL, " TP=", newTP, " (from Ask=", Ask, ")"); //--- Log
      }
   } else {
      //--- Sell signal → close Buy (loser), keep Sell (winner)
      if (prt) Print("Hedge: SELL signal — closing Buy leg ticket=", buyTicket); //--- Log
      ClosePositionByTicket(buyTicket);                            //--- Close buy
      buyTicket = 0;                                               //--- Clear
      currentHedgeState = Hedge_SellOnly;                          //--- Update state

      //--- SL/TP from current market price, identical to original EA
      if (IsPositionOpen(sellTicket)) {
         double newSL = NormalizeDouble(fvgHigh + sl_pts * _Point, _Digits); //--- SL same as original
         double newTP = NormalizeDouble(Bid - tp_pts * _Point, _Digits);     //--- TP same as original
         obj_Trade.PositionModify(sellTicket, newSL, newTP);       //--- Modify
         if (prt) Print("Hedge: Sell leg SL=", newSL, " TP=", newTP, " (from Bid=", Bid, ")"); //--- Log
      }
   }
}
...
//+------------------------------------------------------------------+
//| Trade on FVGs — hedge version                                    |
//| Full hedge active → signal → close loser → TP/SL → re-hedge     |
//+------------------------------------------------------------------+
void TradeOnFVGs() {
   double Ask = NormalizeDouble(SymbolInfoDouble(_Symbol, SYMBOL_ASK), _Digits); //--- Get ask
   double Bid = NormalizeDouble(SymbolInfoDouble(_Symbol, SYMBOL_BID), _Digits); //--- Get bid
   for (int j = 0; j < ArraySize(fvgs); j++) {                    //--- Iterate FVGs
      ...
      bool isBuySignal = !fvgs[j].origUp;                         //--- origDown → Bullish IFVG → Buy

      if (currentHedgeState == Hedge_Full) {
         //--- Close the losing leg, apply SL/TP to winner
         if (isBuySignal) {
         ...
         CloseLosingLeg(isBuySignal, fvgLow, fvgHigh);            //--- Close loser

      } else if (currentHedgeState == Hedge_None) {
         //--- No hedge (safety fallback) — open directional trade like original EA
         double lot = CalcLot();                                   //--- Calc lot
         if (isBuySignal) {
            if (prt) Print("BULLISH IFVG TRADE SIGNAL (no hedge) For ", fvgs[j].name, " at ", Bid); //--- Log
            ...
            if (prt) Print("BEARISH IFVG TRADE SIGNAL (no hedge) For ", fvgs[j].name, " at ", Ask); //--- Log
      ...
      } else if (currentHedgeState == Hedge_BuyOnly && !isBuySignal) {
         //--- Sell signal while Buy running → close Buy, open Sell
         if (prt) Print("BEARISH IFVG: Closing Buy, opening Sell for ", fvgs[j].name); //--- Log
         ClosePositionByTicket(buyTicket);                         //--- Close buy
         buyTicket = 0;                                            //--- Clear
         double lot = CalcLot();                                   //--- Calc lot
         double SL_Sell = NormalizeDouble(fvgHigh + sl_pts * _Point, _Digits); //--- Calc sell SL
      ...
      } else if (currentHedgeState == Hedge_SellOnly && isBuySignal) {
         //--- Buy signal while Sell running → close Sell, open Buy
         if (prt) Print("BULLISH IFVG: Closing Sell, opening Buy for ", fvgs[j].name); //--- Log
         ClosePositionByTicket(sellTicket);                        //--- Close sell
         sellTicket = 0;                                           //--- Clear
         double lot = CalcLot();                                   //--- Calc lot
         double SL_Buy = NormalizeDouble(fvgLow - sl_pts * _Point, _Digits); //--- Calc buy SL
      ...
      //--- Same-direction signal while single leg running → ignore (already positioned)
...
//+------------------------------------------------------------------+
//| Apply Points Trailing Stop (only on exposed legs, not hedge)     |
//+------------------------------------------------------------------+
   ...
   for (int i = PositionsTotal() - 1; i >= 0; i--) {              //--- Iterate positions reverse
      if (PositionGetTicket(i) > 0) {                             //--- Check valid ticket
         if (PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == magic_number) { //--- Check symbol and magic
            ...
            //--- Skip hedge legs (no SL/TP set = still hedged)
            if (PositionGetDouble(POSITION_SL) == 0 && PositionGetDouble(POSITION_TP) == 0) continue; //--- Skip hedge
...
int OnInit() {
   ...
   buyTicket = 0;                                                 //--- Reset buy ticket
   sellTicket = 0;                                                //--- Reset sell ticket
   currentHedgeState = Hedge_None;                                //--- Reset hedge state
   ...
   //--- Open initial full hedge
   if (prt) Print("Opening initial full hedge...");                //--- Log
   OpenFullHedge();                                                //--- Open hedge
...
//+------------------------------------------------------------------+
void OnTick() {
   //--- Always check if positions are still alive (TP/SL can hit any tick)
   UpdateHedgeState();                                             //--- Update hedge
   ...
   //--- Trailing only on exposed legs (not while fully hedged)
   if (TrailingType == Trailing_Points && currentHedgeState != Hedge_Full && PositionsTotal() > 0) { //--- Check trailing

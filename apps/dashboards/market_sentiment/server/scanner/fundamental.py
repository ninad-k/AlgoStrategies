...
def _eps_trend(ticker: yf.Ticker, info: dict) -> str:
    ...
    try:
        # Use income_stmt (quarterly) to derive EPS — Ticker.earnings is removed in yfinance 1.x
        stmt = ticker.quarterly_income_stmt
        if stmt is not None and not stmt.empty:
            net_income_row = next(
                (r for r in ("Net Income", "Net Income Common Stockholders") if r in stmt.index),
                None,
            )
            shares_row = next(
                (r for r in ("Diluted Average Shares", "Basic Average Shares") if r in stmt.index),
                None,
            )
            if net_income_row and shares_row:
                ni     = stmt.loc[net_income_row].dropna().sort_index()
                shares = stmt.loc[shares_row].dropna().sort_index()
                common = ni.index.intersection(shares.index)
                if len(common) >= 2:
                    eps = ni[common] / shares[common]

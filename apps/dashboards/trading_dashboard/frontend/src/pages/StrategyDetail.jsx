...
import { getStrategyTrades, getStrategyRisk, getPnlBySymbol } from '../api/pnl'
import { getStrategies } from '../api/config'
...
export default function StrategyDetail() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [strategies, t, r, sym] = await Promise.all([
        getStrategies(),
        getStrategyTrades(id, filters),
        getStrategyRisk(id, filters),
        getPnlBySymbol({ ...filters, category_type: 'strategy', category_id: id }),
      ])
      setStrategy(strategies.find(s => s.id === parseInt(id)))
  ...
  return (
    <AppShell>
      <TopBar title={strategy?.name ?? `Strategy #${id}`}>
        ...
        {loading ? <LoadingSpinner /> : (
          <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <StatCard label="Magic Number" value={strategy?.magic_number ?? '—'} />
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="rounded-lg border border-gray-200 p-4 bg-white">
                  ...
                  <div className="mt-1">
                    {risk?.avg_rr_deviation != null
                      ? <Badge label={`${risk.avg_rr_deviation >= 0 ? '+' : ''}${risk.avg_rr_deviation.toFixed(2)}`} variant={risk.avg_rr_deviation >= 0 ? 'green' : 'red'} />
                      : '—'}
                  </div>
                </div>

"""Embedded TOOL SELECTION GUIDE handed to MCP clients on startup.

The string below is sent verbatim as the server's ``instructions`` field. The
content steers the calling model toward the cheapest and most reliable tool
sequence: read state once, scope queries with ``study_filter``, prefer
``summary=true`` over raw payloads, and reuse entity ids across a session.

Keep this in sync with the original Node project's guide so models that learned
the JS server's vocabulary keep working against this Python port.
"""

from __future__ import annotations

TOOL_SELECTION_GUIDE: str = """\
TradingView MCP — tools for reading and controlling a live TradingView Desktop chart.
